  ...
  useEffect(() => {
    ...
    fetch(
      "https://api.nasa.gov/neo/rest/v1/feed?start_date=" +
        ...
        "&end_date=" +
        today +
        "&api_key=7fX5ch29dBRLQKQVxaI7twG9p1ZVmeNY1dSVaSsg"
    )
      .then((response) => response.json())
      .then((data) => {
        const values = Object.values(data.near_earth_objects);
        values[0].forEach((object) => {
          newBubbleChartData.x.push(
            object.close_approach_data.close_approach_date_full
          );
          newBubbleChartData.y.push(
            object.close_approach_data.miss_distance.kilometers
          );
          newBubbleChartData.marker.size.push(
            object.estimated_diameter.meters.estimated_diameter_min * 0.5
          );
